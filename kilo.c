...
void editorRowCxToRx(eros *row, int cx) {
    int rx = 0;
    for (int j = 0; j < cx; j++) {
        if (row->chars[j] == '\t') {
...
void editorRowDelChar(eror* row, int at) {
...
void editorFindCallback(char* query, int key) {
    ...
    for (i = 0; i < E.numrows; i++) {
        ...
        char* match = strstr(row_render, query);
        if (match) {
            last_match = current;
            E.cy = current;
...
void editorDrawStatusBar(struct abuf *ab) {
    abAppend(ab, "\x1b[7m", 4); // invert colors for bar
    ...
    abAppend(ab, "\x1b[m", 3);// switch back to normal color scheme
    abAppend(ab, "\r\n", 2);
}
